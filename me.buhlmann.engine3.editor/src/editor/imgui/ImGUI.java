...
public abstract class ImGUI {
  public static final Map<Class<?>, ImGUI> guis = new ConcurrentHashMap<>();
  public static final Map<Class<?>, ImGUI> globals = new ConcurrentHashMap<>();
  ...
  public static void add(ImGUI gui) {
    if (!ImGUI.guis.containsKey(gui.getClass())) {
      gui.onInit();
      ImGUI.guis.put(gui.getClass(), gui);
    }
  }

  public static void addGlobal(ImGUI gui) {
    if (!ImGUI.guis.containsKey(gui.getClass())) {
      gui.onInit();
      ImGUI.globals.put(gui.getClass(), gui);
    }
  }
  ...
  public static void renderGUIS(IScene scene, IRenderer renderer) {
    ...
    for (ImGUI gui : ImGUI.globals.values()) {
      gui.render(scene, renderer);
    }
  }
  ...
  public static void postFrame() {
    ImGui.render();
    ImGui.endFrame();
    ImGUI.ui().renderDrawData(ImGui.getDrawData());
  }
  ...
  public static void initialize(long display) {
    ...
    final ByteBuffer font = Memory.load("platform/resources/fonts/Arial.ttf");
    assert font != null;
    ...
    //ImGui.buildFontAtlas(fontAtlas);
